...
{
    [TestClass]
    public class BaseSqlOperationTests
    {
        ...
        [TestMethod]
        public void Test_Select_WhenNoRecordSuccess()
        {
            dbDataReaderMock.Setup(s => s.Read()).Returns(false);

            dbCommandMock.Setup(s => s.ExecuteReader()).Returns(dbDataReaderMock.Object);

            var result = sqlOperationService.Object.Select<SessionDto>(SelectSqlQuery.GetAll_Session);
